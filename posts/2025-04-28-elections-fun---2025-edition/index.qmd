...
This shows that overall the Liberals flipped `r flips$Liberal` seats and the Conservatives flipped `r flips$Conservative` in their favour, while the liberals lost `r flips_lost$Liberal` and the Conservatives lost `r flips_lost$Conservative` seats compared to the redistributed 2021 votes. No other parties picked up seats. The biggest losers in this election were the NDP who lost `r flips_lost$NDP` seats, while the Bloc Québécois lost `r flips_lost[["Bloc Québécois"]]` seats and the Green Party lost `r flips_lost[["Green Party"]]` seat.


```{r eval=FALSE, include=FALSE}
map_data3 <- map_data2 |> 
  st_cast("POLYGON") |> # only keep largest polygon for each district
  mutate(a=st_area(geometry)) |>
  slice_max(order_by = a, n=1, with_ties = FALSE,by=FEDUID) 

hex <- map_data3 |>
  geogrid::calculate_grid(grid_type = "hexagonal", seed = 1, verbose = FALSE, learning_rate = 0.03)

hexmap <- geogrid::assign_polygons(map_data3, hex)

hexmap |>
  mutate(p=if_else(flip,Party,"Other")) |>
  ggplot() + 
  geom_sf(aes(fill = p)) +
  scale_fill_manual(values=party_colours) +
  coord_sf(datum=NA) 
