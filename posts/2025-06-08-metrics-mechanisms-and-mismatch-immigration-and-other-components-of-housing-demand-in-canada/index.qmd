...
```{r fig-immigration-status-housing-consumption, fig.height=4}
bind_rows(
  pumf_data |>
...
```{r fig-immigration-status-housing-consumption-no-children,fig.height=4}
...
```{r fig-age-adjusted-rates, include=FALSE, eval=FALSE}
weights <- pumf_data |> select(matches("WEIGHT|WT\\d+")) |> names()
age_dist <- pumf_data |>
  filter(!dependen_child) |>
  filter(Year=="2021") |>
...
bind_rows(
  ...
  filter(!dependen_child) |>
  summarize(across(matches("WEIGHT|WT\\d+"),sum),
            .by=c(Year,IMMSTAT,PRIHM,AGEGRP)) |>
  pivot_longer(matches("WEIGHT|WT\\d+"),
               names_to="Weight",
               values_to="Count") |>
  mutate(rate=Count/sum(Count)*1000,.by=c(Year,IMMSTAT,Weight,AGEGRP)) |>
  mutate(rate=coalesce(rate,0)) |>
  filter(PRIHM=="Person is primary maintainer") |>
  select(-PRIHM) |>
  left_join(age_dist,by="AGEGRP") |>
  summarize(rate=weighted.mean(rate,Population),.by=c(Year,IMMSTAT,Weight)) |>
  ggplot(aes(y=IMMSTAT,x=rate,fill=Year)) +
  geom_col(position="dodge",
           data=~filter(.,Weight=="WEIGHT")) +
  geom_errorbar(position=position_dodge(width = .9),
  ...
  scale_fill_manual(values=year_colours) +
  guides(fill=guide_legend(reverse = TRUE)) +
  labs(title="Age-adjusted housing occupancy by immigration status",
       y=NULL,x="Age-adjusted number of housing units occupied per 1,000 population\n(excluding dependent children under 20 years old, age-adjusted to 2021 Canadian population)")
```
...
```{r fig-immigrant-household-type}
...
hh_sizes |>
  ...
  ggplot(aes(x=Date,y=pop/hh,colour=Year)) +
  geom_line() +
  geom_vline(xintercept=as.Date(c("2024-07-01","2034-07-01")),linetype="dashed") +
  labs()
```

```{r fig-demographic-contribution,fig.height=4.5}
...
We have done some work to estimate the impact of income growth on housing demand, but have not explicitly solved for this. For key household formation age groups the impact of income on household formation, controlling for couple status and age, and with it age-related increases in income, is about `r income_to_rent_elasticity` times that of rent, with the opposite sign.[^6] [@mhu.2025] This means that in the Vancouver setting a 1% increase in real incomes results to an increase in household demand by about `r scales::percent(income_elasticity, accuracy=0.1)`. For all of Canada this is likely an over-estimate, and assuming the housing shortfall is reduced in Vancouver, it would also reduce Vancouver's elasticity. A back of the envelope guesstimation suggests that maybe reducing this by a factor of 1.5 or even 2 might give a reasonable ballpark estimate. For the sake of this post we will use a fudge factor of `r round(1/fudge_factor,digits=2)` to reduce the elasticity to `r scales::percent(income_elasticity_f, accuracy=0.1)`.
...
Historically real age-adjusted family incomes in Canada have grown by about `r scales::percent(income_growth, accuracy=0.1)` per year, so we can expect, including our fudge factor, a `r scales::percent(household_growth_income_f, accuracy=0.01)` increase in household demand per year, or about `r scales::percent(household_growth_income_f_10, accuracy=0.1)` over the next decade. Using this ballpark estimate, that results in demand for roughly an additional `r scales::comma(additional_hh,accuracy=10000)` households from income growth as a ballpark estimate. Overall, the impact of income growth on housing demand remains under-studied and could use further research.
...
These previous sections provided various estimates on household growth or demand. To fully translate this into housing growth we need to add in moving vacancies. Generally a `r scales::percent(moving_vacancies)` bump to all estimates is a good ballpark to ensure people can move freely and housing is available. In the past planning has often skipped this step, effectively resulting in net attrition. This is not the largest problem with past past planning practices when it comes to projections for housing demand, but it's a simple one to fix.[^10]
...
Putting all the estimates above together we get demand of about `r scales::comma(total_growth, accuracy=100000)` additional households from demographic shifts, immigration, and income growth over the next decade. Adding this to the existing housing shortage gets us to `r scales::comma(total_with_shortfall[1], accuracy=100000)` to `r scales::comma(total_with_shortfall[2], accuracy=100000)` additional households. Factoring in moving vacancies to translate this into housing units bumps that up to `r scales::comma((total_growth+shortfall_low)*(1+moving_vacancies), accuracy=100000)` to `r scales::comma((total_growth+shortfall_high)*(1+moving_vacancies), accuracy=100000)` additional net new housing units needed. Accounting for teardowns and deconversions, roughly estimated at `r scales::percent(teardown_rate)` of new construction, we arrive at a need between `r scales::comma(total_with_shortfall_vacancy_teardown[1], accuracy=100000)` and `r scales::comma(total_with_shortfall_vacancy_teardown[2], accuracy=100000)` housing completions over the next decade.
...
```{r fig-housing-demand-components,fig.height=4}
demand_components |>
  mutate(gap=high-low) |> 
...
```{r fig-cananda-centres-completions,fig.height=4.5}
...
```{r fig-new-repair-construction, fig.height=4.5}
...
```{r fig-immigrant-incomes,fig.height=4}
